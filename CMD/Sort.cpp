...
void flashSort(int a[], int n)
{
    ...
    if (a[max] == minVal)
        return;
    double c1 = ((double)m - (double)1) / ((double)a[max] - (double)minVal);
    int x = 0;
    for (int i = 0; i < n; i++)
    {
        x = (int)((int)c1 * ((int)a[i] - (int)minVal));
        l[x] += 1;
    }
    for (int i = 1; i < m; i++)
        l[i] += l[i - 1];
    ...
    while (nmove < n - 1)
    {
        ...
        {
            j++;
            k = (int)c1 * (int)((int)a[j] - (int)minVal);
        }
        flash = a[j];
        if (k < 0) break;
        while (j != l[k])
        {
            k = (int)c1 * (int)((int)flash - (int)minVal);
            int hold = a[t = --l[k]];
            a[t] = flash;
            flash = hold;
            ++nmove;
...
void flashSort(int a[], int n, long long& d)
{
    int minVal = a[0];
    int max = 0;
    int m = int(0.45 * n);
    ...
    while (++d && nmove < n - 1)
    {
        ...
        {
            j++;
            k = (int)c1 * ((int)a[j] - (int)minVal);
        }
        flash = a[j];
        if (++d && k < 0) break;
        while (++d && j != l[k])
        {
            k = (int)c1 * ((int)flash - (int)minVal);
            int hold = a[t = --l[k]];
            a[t] = flash;
            flash = hold;
            ++nmove;
